...
Authentication replacement validates a candidate API key or OAuth token through model discovery before replacing the owner-only credential file. Setup presents the default api.airforce service or a custom endpoint; custom endpoints accept API keys only, and application validation rejects OAuth credentials against another origin. The interactive command then rebuilds the provider and runtime so the current session uses the new credential. Environment-provided API keys remain an explicit higher-precedence override and cannot be changed in a parent shell by the CLI.
