...
During a request, an animated status shows the current phase and elapsed time: preparing context, waiting for the model, receiving a response, preparing a tool call, or running tools. Command output streams beneath its tool as it runs, with a 16 KB display cap while the complete bounded result remains available to the agent. Escape or Ctrl+C cancels active model and tool work. Assistant responses render terminal-friendly Markdown, including headings, lists, tables, links, blockquotes, inline code, and syntax-highlighted fenced code. Complete ordinary lines render immediately; tables and code fences remain buffered until they can be rendered safely. Hidden model chain-of-thought is never displayed.

An API response stream that produces no data for 60 seconds fails with `STREAM_TIMEOUT`; the overall request deadline reports `API_TIMEOUT`. Both errors suggest retrying or running `airforce doctor`. Session locks contain the owning process ID and are recovered automatically when that process no longer exists.
